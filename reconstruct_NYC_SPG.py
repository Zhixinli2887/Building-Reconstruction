...
def reconstruct_planes(planes, planes_para_all, dem_plane, theta, APD=0.6, simple_th=0.5, alpha=1.0, boundary=None):
    DTM, pv_meshes, tri_meshes = [], [], []
    ashapes, planes_para, planes_ids = [], [], []

    for idx in tqdm(range(len(planes)), desc=f'    Regularizing {len(planes)} planes...'):
        try:
    ...
    planes_reg = []
    for ashpae in ashapes:
        try:
            polys = regularize([ashpae], False, 2, theta)
            for poly in polys:
                planes_reg.append(check_multipolygon(poly, 'max')[0])
        except:
            continue

    # planes_reg = [check_multipolygon(poly, 'max')[0] for poly in regularize(ashapes, False, 2, theta)]
    ...
    planes_reg = [planes_reg[i] for i in ids if not planes_reg[i].is_empty and
                  planes_reg[i].geom_type != 'LineString']
    flags = [True for _ in planes_reg]

    remaining = boundary.difference(cascaded_union(planes_reg))
    if not remaining.is_empty:
        remaining_temp = check_multipolygon(remaining, type_return='all')
        remaining, remaining_para, remaining_flags = [], [], []
        for poly in remaining_temp:
            flag, rst = simplify_by_angle(poly)
            if flag and 0.2 <= rst.area:
            # if flag and 0.2 <= rst.area <= 100:
                remaining.append(rst)

        for poly in remaining:
            try:
                ids = [idx for idx in range(len(planes_reg)) if poly.intersects(planes_reg[idx])]
                zs = [planes_para[idx].point[-1] for idx in ids]
                remaining_para.append(planes_para[ids[np.argmin(zs)]])
            except:
    ...
    for i in range(len(planes_reg)):
        ...
        if flags[i]:
            ...
            vertices = np.array([get_vert_z(p, planes_para[i].point, planes_para[i].normal) for p in vertices])
            # vertices = np.array([get_vert_3D(p, verts_all[:, 0:3]) for p in vertices])
    ...
    #             faces_all.append(faces)
    #
    #         clusters = DBSCAN(eps=0.5, min_samples=2).fit(verts_all[:, 0:3])
    #         for lbl in np.unique(clusters.labels_):
    #             if lbl != -1:
    #                 verts_all[lbl == clusters.labels_, 2] = verts_all[lbl == clusters.labels_, 2].max()
    #
...
if __name__ == "__main__":
    ...
    APD, lod, alpha = 1, 2, 0.5
    # RANSAC parameters
    sp, mnd, eps = 50, 20, 1.0
    ...
    for fp in pc_fps:
        ...
        if os.path.exists(out_mesh_fp):
            ...
            planes, paras = facade_remove(ransac_cc(pc_raw, sp=sp, mnd=mnd, eps=eps), th=30)
            ...
            if len(planes) == 0:
                ...
                try:
                    ...
                    _, theta, IOU = get_domain_direction([pc_raw], simple_th=APD, alpha=alpha)
                ...
                # Domain direction computation
                boundary, theta, IOU = get_domain_direction([pc_raw], simple_th=APD, alpha=alpha)
                thetas = get_thetas(theta, lod)
                # Plane resampling
                    ...
                    if not iter_flag:
                        if len(planes_src) > 0:
                            plane_DTM, pv_meshes, tri_meshes = reconstruct_planes(planes_src, paras, dem_plane, theta,
                                                                                  APD=APD, simple_th=APD, alpha=alpha,
                                                                                  boundary=boundary)
