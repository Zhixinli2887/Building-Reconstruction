...
def reconstruct_planes(planes, planes_para_all, dem_plane, theta, APD=0.6, simple_th=0.5, alpha=1.0):
    ...
    for nodes in tqdm(planes_cluster, desc=f'    Regularizing {len(planes)} planes...'):
        ...
        boundary = regularize(check_multipolygon(alphashape.alphashape(cloud, alpha).simplify(2)),
                              False, 2, theta)[0]
        ...
        for idx in nodes:
            ...
            for shape in shapes:
                ashapes.append(shape.simplify(2).buffer(simple_th / 2, join_style='mitre'))
            ...
            for j in [item for item in ids if item != i]:
                if planes_reg[i].intersects(planes_reg[j]):
                    ...
                    try:
                        ...
                        planei = cloud_i[np.random.randint(len(cloud_i), size=5000)]
                        planej = cloud_j[np.random.randint(len(cloud_j), size=5000)]
                        dists = cdist(planej, planei)
...
def get_domain_direction(clouds, simple_th=0.5, alpha=1.0):
    ...
    ashape = check_multipolygon(alphashape.alphashape(clouds[:, 0:2], alpha).simplify(2))
...
if __name__ == "__main__":
    ...
    for fp in pc_fps:
        ...
        if os.path.exists(out_mesh_fp):
            ...
            planes, paras = facade_remove(ransac_cc(pc_raw, sp=300, mnd=25, eps=0.4), th=15)
            ...
            if len(planes) == 0:
                ...
                # planes, paras = facade_remove(ransac_sac(pc_raw, sp=350, eps=0.15), th=15)

            if len(planes) == 0:
                print('  No planes found, use primitive for fitting.')
                try:
                    # Domain direction computation
                    _, theta, IOU = get_domain_direction([pc_raw], simple_th=APD, alpha=0.5)
                    ...
                    if not iter_flag:
                        if len(planes_src) > 0:
                            plane_DTM, pv_meshes, tri_meshes = reconstruct_planes(planes_src, paras, dem_plane, theta,
                                                                                  APD=APD, simple_th=APD, alpha=0.5)
