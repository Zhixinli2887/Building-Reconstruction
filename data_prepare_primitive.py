...
if __name__ == "__main__":
    sigma, RMSEs = 0.0, []
    data_fd = f'data/roof_primitive_perfect_{int(sigma * 100)}cm'
    ...
    data_num, pt_num, train_id, test_id = 10000, 512, 0, 0
    ...
    for roof in roof_types:
        for i in tqdm(range(int(data_num / cls_num)), desc=f'Generating {roof}...'):
            ...
            # w, l = np.random.uniform(10, 50, 2)
            # h = np.random.uniform(4, 10)
            w, l, h = np.random.uniform(3, 6, 3)
            wr, lr = np.random.uniform(0.3, 0.9, 2)

            # noise = np.random.normal(0, sigma, int(pt_num / 4 * 3))
            ...
            if occlusion_prob < -1 and primitive_dict[roof]['miss']:
            ...
            # sample_edge = sample_edge[np.random.choice(sample_edge.shape[0], int(pt_num / 4), replace=False)]
            ...
            # sample = geom.sample_points_uniformly(int(pt_num / 4 * 3), True)
