...
def shed_geom(x, face_merge=False, return_paras=True):
    w, l, _, _, h = x
    ...
    if return_paras:
        return verts, roof_faces, bottom_faces, [w, l, 1, 1, h]
    else:
        return verts, roof_faces, bottom_faces
...
def flat_geom(x, face_merge=False, return_paras=True):
    w, l, _, _, _ = x
    verts = np.array([[- w / 2, + l / 2, 0],
                      [+ w / 2, + l / 2, 0],
                      [+ w / 2, - l / 2, 0],
                      [- w / 2, - l / 2, 0]])
    roof_faces = [[[0, 2, 1],
    ...
    if return_paras:
        return verts, roof_faces, bottom_faces, [w, l, 1, 1, 0]
    else:
        return verts, roof_faces, bottom_faces


def box_geom(x, face_merge=False, return_paras=True):
    w, l, _, _, h = x
    verts = np.array([[- w / 2, + l / 2, h / 2],
                      [+ w / 2, + l / 2, h / 2],
                      [+ w / 2, - l / 2, h / 2],
...
if __name__ == "__main__":
    ...
    w, l, h, wr, lr = 2, 2, 2, 0.3, 0.7
    x = [w, l, wr, lr, h]
    shed = shed_geom(x, True, False)

    faces_pl = [[3, face[0], face[1], face[2]] for face in shed[1]]

    pl = pv.Plotter()
    pl.set_background('grey')
    pl.add_mesh(pv.PolyData(shed[0], faces_pl), opacity=0.999, show_edges=False)
    geom_mesh = pv.PolyData(shed[0], faces_pl)
    ...
    pl.show()
    #
    # curve = round_geom(x, True, False)
    # pyramid = pyramid_geom(x, True, False)
    # gable = gable_geom(x, True, False)
    # hip = hip_geom(x, True, False)
    # mansard = mansard_geom(x, True, False)
    # shed = shed_geom(x, True, False)
    # flat = shed_geom(x, True, False)
    # cylinder = round_geom(x, True, False)
    # semi_pyramid = semi_pyramid_geom(x, True, False)
    # semi_hip = semi_hip_geom(x, True, False)
    # semi_mansard = semi_mansard_geom(x, True, False)
    # saltbox = saltbox_geom(x, True, False)
    # gambrel = gambrel_geom(x, True, False)
    #
    # pl = pv.Plotter(shape=(2, 5))
    #
    # for cid, geom in enumerate([pyramid, gable, hip, mansard, cylinder]):
    #     pl.subplot(0, cid)
    #     faces_pl = [[3, face[0], face[1], face[2]] for face in geom[1]]
    #     verts = pv.PolyData(geom[0])
    #     verts["Labels"] = [f"V {i}" for i in range(verts.n_points)]
    #     pl.add_mesh(pv.PolyData(geom[0], faces_pl), opacity=0.85, show_edges=False)
    #     geom_mesh = pv.PolyData(geom[0], faces_pl)
    #     edges = geom_mesh.extract_feature_edges(
    #         boundary_edges=True,
    #         non_manifold_edges=False,
    #         feature_angle=10,
    ...
    #     pl.add_mesh(edges, color='k', line_width=3)
    #
    # for cid, geom in enumerate([saltbox, gambrel, semi_pyramid, semi_hip, semi_mansard]):
    #     pl.subplot(1, cid)
    #     faces_pl = [[3, face[0], face[1], face[2]] for face in geom[1]]
    #     pl.add_mesh(pv.PolyData(geom[0], faces_pl), opacity=0.85, show_edges=False)
    #     verts = pv.PolyData(geom[0])
    ...
    # for geom in [saltbox]:
    #     mesh = o3d.geometry.TriangleMesh()
    #     mesh.vertices = o3d.utility.Vector3dVector(geom[0])
    #     mesh.triangles = o3d.utility.Vector3iVector(geom[1])
