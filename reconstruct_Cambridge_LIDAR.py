...
if __name__ == "__main__":
    ...
    for fp in pc_fps:
        ...
        if os.path.exists(out_mesh_fp):
            ...
            planes, paras = facade_remove(ransac_cc(pc_raw, sp=30, mnd=20, eps=0.15))
            if len(planes) == 0:
                print('  CloudCompare RANSAC Failed, using pyransac3D RANSAC...')
                planes, paras = facade_remove(ransac_sac(pc_raw, sp=30, eps=0.15))
