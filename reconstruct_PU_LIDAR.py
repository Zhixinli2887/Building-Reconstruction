...
def reconstruct_planes(planes, planes_para_all, dem_plane, theta, APD=2.40, simple_th=0.5, alpha=1.0):
    ...
    for nodes in tqdm(planes_cluster, desc=f'    Regularizing {len(planes)} planes...'):
        ...
        clusters = DBSCAN(eps=2 * APD, min_samples=2).fit(verts_all[:, 0:3])
...
if __name__ == "__main__":
    ...
    pc_fps = [os.path.join(pc_fd, item) for item in os.listdir(pc_fd) if 'lasx' not in item and '298' in item]
    # Loading DEM
    print('Loading DEM...')
    ds = gdal.Open(dem_fp)
    ...
    for fp in pc_fps:
        ...
        if os.path.exists(out_mesh_fp):
            ...
            planes, paras = facade_remove(ransac_cc(pc_raw, sp=25, mnd=20, eps=0.6, sample_res=-1), th=15)
            # pl = pv.Plotter()
            # for cloud in planes:
            ...
            if len(planes) == 0:
                ...
                planes, paras = facade_remove(ransac_cc(pc_raw, sp=25, mnd=20, eps=0.6, sample_res=1.5), th=15)

            if len(planes) == 0:
                print('  No planes found, use primitive for fitting.')
                try:
                    pc_height = plane_coef.distance_points(pc_raw)
                    pc_raw = pc_raw[pc_height > 2 * feet]
                    ...
                    print('  Primitive fitting failed, skip.')
                    continue
            else:
                # Domain direction computation
                _, theta, IOU = get_domain_direction(planes, simple_th=APD, alpha=0.2)
                thetas = get_thetas(theta, lod)
                ...
                primitive_geoms, iter_num = [], 0
                try:
                    planes_sample = smaple_planes(planes)
                    iter_flag = True
                except:
                    print('  Plane resampling failed, skip.')
                    iter_flag = False
                while iter_flag:
                    ...
                    if len(candidates) == 0:
                        ...
                        if DTM < 0.015 and MTD < 0.18:
                            ...
                            # Outliers removal
                            npids = get_outliers(planes, verts, faces, proposals[bid], geom_DTM)
                    ...
                    if not iter_flag:
                        if len(planes_src) > 0:
                            plane_DTM, pv_meshes, tri_meshes = reconstruct_planes(planes_src, paras, dem_plane, theta,
                                                                                  APD=APD, simple_th=APD, alpha=0.2)
